#!/usr/bin/env python3
"""Validate the HolyToy Shadertoy v2 corpus without compiling its sources.

v2 has two strata with per-stratum input gates:
  * single_no_channels     - inputs empty AND no texture/channel operation in
                             the comment-stripped source (v1's purity gate;
                             comments are stripped first because two accepted
                             pure shaders mention the word "texture" in prose).
  * single_texture_channels - inputs nonempty, every input a static texture
                             channel; VR/image-store operations still banned.
"""
...
ALWAYS_DISALLOWED = re.compile(r"\b(?:mainvr|imageload|imagestore|texturecube)\b", re.I)
STRATA = ("single_no_channels", "single_texture_channels")
...
for item in records:
    ...
    for pass_value in project_passes:
        ...
        stratum = item.get("coverage", {}).get("primary_stratum")
        if stratum not in STRATA:
            fail(f"{shader_id}: unknown stratum {stratum!r}")
        stripped = strip_comments(raw.decode("utf-8-sig"))
        inputs = pass_value.get("inputs") or []
        if stratum == "single_no_channels":
            forbidden_auxiliary = sorted(set(match.group(0) for match in PURE_DISALLOWED.finditer(stripped)))
            if forbidden_auxiliary:
                fail(f"{relative}: prohibited auxiliary texture/channel operation(s): {forbidden_auxiliary}")
            if inputs:
                fail(f"{relative}: single_no_channels pass must not declare inputs")
        elif stratum == "single_texture_channels":
            forbidden_always = sorted(set(match.group(0) for match in ALWAYS_DISALLOWED.finditer(stripped)))
            if forbidden_always:
                fail(f"{relative}: prohibited operation(s) for texture stratum: {forbidden_always}")
            if not inputs:
                fail(f"{relative}: single_texture_channels pass must declare its texture inputs")
        ...
        for input_value in inputs:
            ...
            if input_value.get("type") != "texture":
                fail(f"{relative}: input channel {channel} is not a static texture")
            if not input_value.get("filepath"):
                fail(f"{relative}: input channel {channel} has no recorded media path")
