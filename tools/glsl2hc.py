...
small GLSL subset (docs/VISION.md:45-51) to HolyC targeting the shader ABI
  ...
  U0 MainImage(CHtUniforms *u, F64 frag_x, F64 frag_y,
               CHtFragColor *out_color);  // one RGBA fragment
...
class Emitter:
    ...
    def emit_main_image(self, fn):
        ...
        if self.runner == "static":
            self.taken.update(("u", "x", "y", "out_color",
                               "ht_" + "lum", "ht_done"))
        else:
            self.taken.update(("u", "frag_x", "frag_y", "out_color", "ht_done"))
        ...
        if self.runner == "static":
            ...
            # Standalone runner retains its original coordinate path.
            self.put("%s=x+0.5;" % coord_names[0])
            self.put("%s=u->res_y-y-0.5;" % coord_names[1])
        else:
            # App ABI mouse positions are raw viewport coordinates, Y-down.
            self.put("ht_iMouse_y=u->res_y-u->mouse_y;")
            self.put("if (u->click_x<0) {")
            self.put("  ht_iMouse_z=0.0; ht_iMouse_w=0.0;")
            self.put("} else if (u->mouse_lb) {")
            self.put("  ht_iMouse_z=u->click_x; ht_iMouse_w=u->res_y-u->click_y;")
            self.put("} else {")
            self.put("  ht_iMouse_z=-u->click_x; ht_iMouse_w=-(u->res_y-u->click_y);")
            self.put("}")
            # Renderer already supplies bottom-left, pixel-centered coordinates.
            self.put("%s=frag_x;" % coord_names[0])
            self.put("%s=frag_y;" % coord_names[1])
        ...
        if self.main_done_used:
            self.put("ht_done:;")
        if self.runner == "static":
            self.put("ht_" + "lum=Clamp(0.299*%s+0.587*%s+0.114*%s,0.0,1.0);"
                     % tuple(color_names[:3]))
            self.put("*out_color" + "=ClampI64(ToI64(ht_" +
                     "lum*16.0),0,15);")
            header = "U0 MainImage(CHtUniforms *u,I64 x,I64 y,U8 *out_color)"
        else:
            self.put("out_color->r=%s;" % color_names[0])
            self.put("out_color->g=%s;" % color_names[1])
            self.put("out_color->b=%s;" % color_names[2])
            self.put("out_color->a=%s;" % color_names[3])
            header = ("U0 MainImage(CHtUniforms *u,F64 frag_x,F64 frag_y,"
                      "CHtFragColor *out_color)")
        self.in_main = False
        self.finish_function(header)
