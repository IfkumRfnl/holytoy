...
# Polls mtime (no inotify dependency). Every iteration prints and retains a
# new RUN_DIR. Ctrl-C to stop.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="${1:?usage: watch.sh SRC.HC}"
[ -f "$SRC" ] || { echo "watch.sh: no such file: $SRC" >&2; exit 2; }

echo "watch.sh: watching $SRC (each run prints its RUN_DIR; Ctrl-C to stop)"
LAST=""
while true; do
    ...
    if [ "$M" != "$LAST" ]; then
        ...
        env -u RUN_DIR "$ROOT/tools/run.sh" "$SRC"
